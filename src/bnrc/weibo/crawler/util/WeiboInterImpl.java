...
public class WeiboInterImpl implements WeiboInter
{
	...
	public User getUserById(String userId)
	{
		...
		catch(Exception e)
		{
			...
			{
				logger.error("Error Code: " + ((WeiboException) e).getErrorCode() + " : " + ((WeiboException) e).getError());
				if(((WeiboException) e).getErrorCode() == 10023) // 用户请求频次超过上限
				{
					logger.error("用户请求频次超过上限");
					AccessToken.getOneAccessToken();
					return getUserById(userId);
				}
				else if(((WeiboException) e).getErrorCode() == 10022) // IP请求频次超过上限
				{
	...
	public String[] getFollowersIdsByUserId(String userId)
	{
		...
		catch(Exception e)
		{
			...
			{
				logger.error("Error Code: " + ((WeiboException) e).getErrorCode() + " : " + ((WeiboException) e).getError());
				if(((WeiboException) e).getErrorCode() == 10023)
				{
					logger.error("用户请求频次超过上限");
					AccessToken.getOneAccessToken();
					return getFollowersIdsByUserId(userId);
				}
				else if(((WeiboException) e).getErrorCode() == 10022)
				{
					logger.error("IP请求频次超过上限");
					try
					{
		...
		catch(Exception e)
		{
			...
			if(e instanceof WeiboException)
			{
				...
				if(((WeiboException) e).getErrorCode() == 10023)
				{
					logger.error("用户请求频次超过上限");
					AccessToken.getOneAccessToken();
					return getStatusesByUserId(userId);
				}
				else if(((WeiboException) e).getErrorCode() == 10022)
				{
					logger.error("IP请求频次超过上限");
